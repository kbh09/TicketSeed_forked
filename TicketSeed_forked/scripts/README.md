...
```mermaid
flowchart LR
    01["▶ load_csv(path)<br/>csv 불러오기"]
    02["▶ clean_data(df, column_list)<br/>normalize_values()를 통해<br/>결측치 및 구분자(, |) 제거,<br/>숫자형일 경우 타입 지정"]
    03["▶ load_csv(path)<br/>csv 불러오기"]
    04["🧹 REFACTOR<br/>clean up,<br/>tests untouched"]
    05["🛡️ GAUNTLET<br/>run every check"]
    06["📊 EVIDENCE<br/>you read the report,<br/>not the code"]
    01 --> 02 --> 03 --> 04 --> 05 --> 06
```
